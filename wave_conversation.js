...
var speech_to_text = new SpeechToTextV1({
  ...
  language_customization_id: config.STTCustomizationid    // comment out this line if you are not using any customized language model
});
...
var assistant = new WatsonAssistantV2({
  ...
  url: config.Assist_URL,
  version: '2018-02-16'
});

assistant.createSession({
  assistant_id: config.AssistId
}).then(res => {
  console.log('Watson Assistant Session is ready')
  assistant_session_id = res.session_id
}).catch(err => {
  console.log(err);
});
...
textStream = micInputStream.pipe(speech_to_text.recognizeUsingWebSocket(recognizeparams));
...
to watson assistant to get appropriate response. You can change it to something else if needed.
Once the attention word is detected,the text is sent to Watson Assistant for processing. The response is generated by Watson Assistant and is sent back to the module.
*/
var assistantcontext = {} ; // Save information on assistant context/stage for continous conversation
var assistant_session_id; // Save id on assistant session 

textStream.on('data', function(str) {
  console.log(' ===== Speech to Text ===== : ' + str); // print the text once received
  if (!assistant_session_id) {
     console.log('Watson Assistant session id is empty. Please hang on for a while');
     return
  }

  var res = str ;
  console.log("msg sent to watson assistant:" ,res);
  assistant.message({
    assistant_id: config.AssistId,
    ...
    if (err) {
      ...
      assistantcontext = response.context ; //update assistant context
      assistant_response =  response.output.generic[0].text  ;
      if (assistant_response != undefined ){
        var params = {
        ...
        console.log("Result from watson assistant : " , assistant_response);
        var matchedintent =  response.output.intents[0].intent ; // intent with the highest confidence
        var intentconfidence = response.output.intents[0].confidence  ;
        console.log("intents : " , response.output.intents) ;
        ...
        */
        ...
        setLEDColor("red", 255);
        console.log("The response (output) text from your watson assistant is empty. Please check your watson assistant flow \n" + JSON.stringify( response))
      }
